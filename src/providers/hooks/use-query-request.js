...
*/
function useQueryRequest(pathname, { fetchOnCall = true, initialData = null, query_params = {} } = {}) {
  const [loading, setLoading] = useState(false);
  const [data, setData] = useState(initialData);
  const [error, setError] = useState("");
  const [reload, setReload] = useState(false);
  const [query, setQuery] = useState(query_params);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}${pathname}?${q}`, {
        method: "GET",
        headers: {
  ...
  return { data, setData, loading, error, refetch, setQuery };
};
